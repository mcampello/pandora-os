...
export async function GET(request: NextRequest) {
  ...
  const { data: existing } = await supabase
    .from("connectors").select("id, credentials")
    .eq("type", "gcalendar").eq("label", email).maybeSingle();
  ...
  if (existing) {
    ...
    await supabase.from("connectors").insert({
      type: "gcalendar", label: email, status: "connected",
      credentials, metadata: { email, name: userInfo.name },
      last_sync_at: new Date().toISOString(),
    });
  }

  return NextResponse.redirect(`${SETTINGS}?connected=${encodeURIComponent(email + " (Calendar)") }`);
}
